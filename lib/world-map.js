...
jvm.WorldMap = function(params) {
  ...
  $(params.container).delegate('.jvectormap-region', 'mouseover mouseout', function(e){
    var path = this,
        ...
        labelShowEvent = $.Event('regionLabelShow.jvectormap'),
    ...
    if (e.type == 'mouseover') {
      ...
      if (!regionOverEvent.isDefaultPrevented()) {
        if (params.hoverOpacity) {
          map.regions[code].set('fill-opacity', params.hoverOpacity);
        }
        if (params.hoverColor) {
          map.regions[code].currentFillColor = map.regions[code].get('fill');
          map.regions[code].set('fill', params.hoverColor);
        }
      }
      ...
      if (params.hoverOpacity) {
        map.regions[code].set('fill-opacity', 1);
      }
      if (map.regions[code].currentFillColor) {
        map.regions[code].set('fill', map.regions[code].currentFillColor);
      }
      map.label.hide();
