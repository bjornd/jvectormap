...
jvm.WorldMap.prototype = {
  ...
  bindElementEvents: function(){
    ...
    this.container.delegate("[class~='jvectormap-element']", 'mouseover mousemove mouseout', function(e){
      var path = this,
          ...
          moveEvent = jvm.$.Event(type+'Move.jvectormap');
      ...
      if (e.type == 'mouseover') {
        ...
        var $c = map.container,
          x = /*e.pageX - $c.offset().left*/ e.offsetX,
          y = /*e.pageY - $c.offset().top*/ e.offsetY,
          latLng = map.pointToLatLng(x, y);
        map.container.trigger(moveEvent, [code, x, y, latLng]);
  ...
  latLngToPoint: function(lat, lng) {
    ...
    //console.log({type:proj.type, lat:lat, lng:lng, centralMeridian:centralMeridian, point:point});
   ...
   * @param {Number} x X-axis of point in map as integer.
   * @param {Number} y Y-axis of point in map as integer.
   * @return {Object} Latitude as x and longitude as y.
   */
  pointToLatLng: function(x, y) {
    ...
    if (inset && jvm.Proj[proj.type + '_inv']) {
      x -= this.transX*this.scale + inset.left*this.scale;
      y -= this.transY*this.scale + inset.left*this.scale;

      bbox = inset.bbox;

      x = (x / (inset.width * this.scale)) * (bbox[1].x - bbox[0].x) + bbox[0].x;
      y = (y / (inset.height * this.scale)) * (bbox[1].y - bbox[0].y) + bbox[0].y;

      return jvm.Proj[proj.type + '_inv'](x, -y, centralMeridian);
    } else {
      //console.log('Error: No inset found or transformation not implemented!');
      return {lat: 0, lng: 0};
    }
  },
...
jvm.WorldMap.maps = {};
jvm.WorldMap.radius = 6378137;  // earth radius as default
jvm.WorldMap.circumference = jvm.WorldMap.radius * Math.PI * 2;   // NOTE: could be removed in favor of 'radius'
