...
jvm.WorldMap.prototype = {
  ...
  setFocus: function(scale, centerX, centerY){
    ...
    if (jvm.$.isArray(scale)) {
      for (var i in scale) {
        if (!this.regions[scale[i]]) {
          continue;
        }
        var thisBbox = this.regions[scale[i]].element.getBBox();
        if (typeof bbox == 'undefined') {
          bbox = thisBbox;
        } else {
          var newX = Math.min(bbox.x, thisBbox.x);
          bbox.width = Math.max(bbox.x + bbox.width, thisBbox.x + thisBbox.width) - newX;
          bbox.x = newX;
          var newY = Math.min(bbox.y, thisBbox.y);
          bbox.height = Math.max(bbox.y + bbox.height, thisBbox.y + thisBbox.height) - newY;
          bbox.y = newY;
        }
      }
    }
